...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if (self) {
        ...
        self.nameLb.font = [UIFont systemFontOfSize:16.5];
        ...
        self.retroLb.textColor = kColorMajor;
        self.retroLb.font = [UIFont systemFontOfSize:15];
        ...
        [self.progressLb mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.size.mas_equalTo(CGSizeMake(kMainScreenWidth - 40, 20));
        }];
...
- (void)setDataWithModel:(RetroListModel *)model {
    self.nameLb.text = [NSString stringWithFormat:@"申请人: %@",model.examineApprove.staff_name];
    for (int i = 0; i < model.examineApproveFillTimes.count; i ++) {
        ...
        if (i == 0) {
            self.retroLb.text = [NSString stringWithFormat:@"考勤状态: %@",timeModel.examine_approve_sign_state];
