...
- (void)setSelected:(BOOL)selected animated:(BOOL)animated
{
    ...
    if (selected)
    {
        if (self.calloutView == nil)
        {
            ...
            nameLab.text = [NSString stringWithFormat:@"姓名: %@", self.detailModel.staff_name];
            ...
            phoneLab.text = [NSString stringWithFormat:@"电话: %@", self.detailModel.staff_phone];
            ...
            self.reportLab =  [[UILabel alloc] initWithFrame:CGRectMake(5 , 5, self.scrollView.width - 10, 50)];
//            if (self.detailModel.report) {
//                self.reportLab.text = self.detailModel.report;
//            }else{
//                self.reportLab.text = self.detailModel.origin;
//            }
            
//            self.reportLab.numberOfLines = 0;
            self.reportLab.textColor = kColorMajor;
...
- (HelpInfoListModel *)infoModel {
    if (!_infoModel) {
        _infoModel = [HelpInfoListModel new];
    }
    return _infoModel;
}
